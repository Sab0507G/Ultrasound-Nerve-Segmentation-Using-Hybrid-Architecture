# ============================================================
# Hybrid U-Net Encoder + SegNet Decoder + Attention Gates
...
def get_paths(data_dir):
    ...
    if len(image_files) != len(mask_files):
        raise RuntimeError(f"Image-mask mismatch: {len(image_files)} vs {len(mask_files)}")
...
def read_pair(img_p, mask_p):
    img = tf.image.resize(tf.image.decode_image(tf.io.read_file(img_p), 3),
                          [IMG_SIZE, IMG_SIZE])
    img = normalize(img.numpy())

    mask = tf.image.resize(tf.image.decode_image(tf.io.read_file(mask_p), 1),
...
def attention_gate(x, g, f):
    ...
    psi   = layers.Activation("sigmoid")(layers.Conv2D(1, 1)(add))
...
def build_model():
    ...
    # Decoder (SegNet + Attention)
    d4 = layers.UpSampling2D()(c5)
    d4 = conv_block(layers.Concatenate()([d4, attention_gate(c4, d4, 512)]), 512)

    d3 = layers.UpSampling2D()(d4)
    d3 = conv_block(layers.Concatenate()([d3, attention_gate(c3, d3, 256)]), 256)

    d2 = layers.UpSampling2D()(d3)
    d2 = conv_block(layers.Concatenate()([d2, attention_gate(c2, d2, 128)]), 128)

    d1 = layers.UpSampling2D()(d2)
    d1 = conv_block(layers.Concatenate()([d1, attention_gate(c1, d1, 64)]), 64)

    outputs = layers.Conv2D(1, 1, activation="sigmoid")(d1)
    return models.Model(inputs, outputs, name="UNet_SegNet_Attention")
...
callbacks_list = [
    callbacks.ModelCheckpoint("best_model.keras", monitor="val_dice_coef",
                              save_best_only=True, mode="max"),
    callbacks.ReduceLROnPlateau(monitor="val_dice_coef", factor=0.5,
                               patience=4, mode="max"),
    callbacks.EarlyStopping(monitor="val_dice_coef", patience=8,
...
model.save("final_unet_segnet_attention.keras")
...
# -------- IoU --------
plt.subplot(1, 3, 3)
plt.plot(history.history['foreground_iou'], label='Train IoU')
plt.plot(history.history['val_foreground_iou'], label='Val IoU')
plt.xlabel('Epoch')
plt.ylabel('IoU')
